...
\be
S(\vec{\ell})^2 = \langle [ X(\vec{\ell}-\delta\vec{\ell}) - X(\vec{\ell}) ]^2 \rangle,
\ee
...
\be
u(\vec{\ell}) = \int e^{i \vec{k} x} d\hat{u}(\vec{k}).
\ee
Square of the transform is called power spectrum,
\be
P(\vec{k}) d\vec{k} = \langle | d\hat{u}(\vec{k}) |^2 \rangle.
\ee
...
\be
S(\ell)^2 = 2\int_0^\infty \left( 1 - \frac{\sin kr}{kr}\right) E(k) dk
\ee
